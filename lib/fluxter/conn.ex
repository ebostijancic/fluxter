defmodule Fluxter.Conn do
  ...
  def write(worker, name, tags, fields, ts)
  when (is_binary(name) or is_list(name)) and is_list(tags) and is_list(fields) do
    ...
    try do
      GenServer.cast(worker, {:write, name, tags, fields, ts})
    catch
      _, _ -> :ok
    end
  end
  ...
  def handle_cast({:write, name, tags, fields, ts}, conn) do
    packet = Packet.build(conn.header, name, tags, fields, ts)
